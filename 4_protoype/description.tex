...
\begin{itemize}
    ...
    \item \textbf{Google Maps API} – \gls{api} zur Verarbeitung und Darstellung geografischer Daten sowie zur Positionsbestimmung innerhalb des Spiels.
    \item \textbf{Google Street View API} – \gls{api} zur Integration von 360-Grad-Street-View-Bildern aus ganz Österreich, welche die Grundlage für das GeoGuessr-ähnliche Spielkonzept bildet.
...
Externe \gls{api}s, wie die Google Maps und Google Street View \gls{api}, werden zur Bereitstellung 
...
Das folgende \gls{erd} zeigt die wichtigsten Datenbanktabellen und deren Beziehungen für die Anwendung \textit{WoSamma}. Es umfasst die zentralen Entitäten wie \texttt{users}, \texttt{profiles}, \texttt{lobbies}, \texttt{rounds} und \texttt{user\_rounds}, sowie die Verknüpfungen zwischen diesen Tabellen, um die komplexen Interaktionen im Mehrspieler-Modus zu unterstützen. Das ERD dient als Grundlage für das Verständnis der Datenstruktur und der Geschäftslogik, die in der Backend-Implementierung mit Supabase realisiert wird.
