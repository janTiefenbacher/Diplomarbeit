...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{content/img/4_Technische_Umsetzung/dbschema.png}
    \caption{Datenbankarchitektur der Anwendung WoSamma (Die vollständige Abbildung der Datenbank führt zu einer eingeschränkten Übersichtlichkeit.)}
    \label{fig:beispielabbildung}
\end{figure}

Die Datenbankarchitektur der Anwendung \textit{WoSamma} basiert auf einem relationalen Schema unter Verwendung von PostgreSQL (über Supabase). 
Im Zentrum des Modells steht die von Supabase bereitgestellte Tabelle \texttt{auth.users}, welche die Authentifizierungsdaten verwaltet. 
Darauf aufbauend werden benutzerspezifische Informationen in der Tabelle \texttt{profiles} gespeichert. 
Diese enthält unter anderem statistische Kennzahlen wie gespielte und gewonnene Spiele sowie Statusinformationen.

Die Spiellogik wird über mehrere miteinander verknüpfte Tabellen abgebildet. 
Die Tabelle \texttt{lobbies} verwaltet aktive Spielräume inklusive Host, Spielstatus und Rundeneinstellungen. 
Über die Zwischentabelle \texttt{lobby\_players} wird die n:m-Beziehung zwischen Benutzern und Lobbys realisiert. 
Einladungen zu Spielrunden werden in \texttt{lobby\_invites} gespeichert.

Einzelne Spielrunden werden in der Tabelle \texttt{user\_rounds} dokumentiert. 
Dort werden unter anderem die tatsächlichen Koordinaten, die vom Benutzer gewählten Koordinaten, die Distanz sowie die erreichten Punkte gespeichert. 
Für den Einzelspielermodus existiert zusätzlich die Tabelle \texttt{user\_daily\_game}, während \texttt{daily\_location} die täglich rotierenden Zielkoordinaten enthält.

Soziale Interaktionen werden durch eigene Tabellen strukturiert: 
\texttt{friend\_requests} bildet Freundschaftsanfragen ab, \texttt{private\_messages} sowie \texttt{news\_messages} ermöglichen Kommunikationsfunktionen. 
Profilbilder werden über \texttt{profile\_pictures} und \texttt{user\_profile\_pictures} verwaltet.

Administrative Prozesse sind ebenfalls integriert. 
Fehlermeldungen werden in \texttt{bug\_reports} gespeichert, während die Tabelle \texttt{reports} Benutzer-Meldungen abbildet. 
Die Tabelle \texttt{config} dient der Speicherung globaler Konfigurationswerte.

Die Datenbank folgt dem Prinzip der Normalisierung, wodurch Redundanzen minimiert und Datenkonsistenz gewährleistet werden. 
Primär- und Fremdschlüssel stellen referenzielle Integrität sicher und ermöglichen eine klare Trennung zwischen Authentifizierungsdaten, Spiellogik, sozialen Funktionen und administrativen Prozessen. 
Durch diese modulare Struktur ist die Datenbank sowohl wartbar als auch skalierbar und kann bei steigenden Nutzerzahlen effizient erweitert werden.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{content/img/4_Technische_Umsetzung/Mockup.jpg}
    \caption{Mockup der Anwendung WoSamma(Überblick)}
    \label{fig:beispielabbildung}
\end{figure}
...
In den folgenden Abschnitten werden die wichtigsten Aspekte des \gls{ui} sowie der Benutzerinteraktion beschrieben. Hierbei
...
Darüber hinaus sorgt die vollständige Ausnutzung des verfügbaren Bildschirmbereichs (height: 100\%) dafür, dass die Karte sowohl auf mobilen Geräten als auch auf größeren Displays konsistent dargestellt wird. Insgesamt trägt diese reduzierte, klar strukturierte \gls{ui} dazu bei, die zentrale Spielhandlung, also das Abgeben einer Standortvermutung, effizient umzusetzen.

Der folgende Codeausschnitt zeigt, wie die Pin-Platzierung technisch umgesetzt wurde:

\begin{lstlisting}[
caption={Pin platzierung auf der Karte},
label={lst:pinplacement}
]
    <script>
      // Globale Variablen:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{content/img/4_Technische_Umsetzung/Pin_setzen.jpeg}
    \caption{platzierter Pin im Einzelspielermodus}
    \label{fig:beispielabbildung}
\end{figure}

\paragraph{Anzeige mehrerer Pins im Mehrspielermodus}
\mbox{}\\
...
\begin{lstlisting}[
caption={Darstellung mehrerer Pins im Mehrspieler-Modus},
label={lst:mehrerepins}
] 
/**
...
\begin{lstlisting}[
caption={Anzeige der Distanz im Einzelspieler-Modus},
label={lst:distanceDisplay}
] 
<View style={styles.resultBox}>
  ...
  <Text style={styles.resultText}>
    Deine Schätzung war ... km vom echten Standort entfernt
  </Text>
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{content/img/4_Technische_Umsetzung/Singleplayer_km.jpeg}
    \caption{Entfernungsanzeige im Einzelspielermodus}
    \label{fig:beispielabbildung}
\end{figure}
...
\begin{lstlisting}[
caption={Anzeige der Distanz im Mehrspieler-Modus direkt auf der Linie},
label={lst:linesDistanceDisplay}
] 
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{content/img/4_Technische_Umsetzung/Mehrspieler_pins.jpeg}
    \caption{Anzeige der Entfernung im Mehrspielermodus}
    \label{fig:beispielabbildung}
\end{figure}
...
Die Zoom-Animation erfüllt dabei nicht nur einen ästhetischen Zweck, sondern unterstützt gezielt die Benutzerführung. Durch das initiale Herauszoomen erhält der Spieler zunächst eine räumliche Gesamtübersicht, wodurch die relative Lage von getippter und tatsächlicher Position im größeren geografischen Kontext sichtbar wird. Das anschließende schrittweise Heranzoomen fokussiert die Aufmerksamkeit dann kontrolliert auf den relevanten Kartenausschnitt. Dieser Ablauf erzeugt eine klare visuelle Dramaturgie und verhindert, dass der Nutzer abrupt in eine stark vergrößerte Detailansicht versetzt wird.

Technisch wird die Animation direkt auf der bestehenden Karteninstanz durchgeführt, indem das Zoom-Level in definierten Intervallen angepasst wird, bis das zuvor berechnete Zielniveau erreicht ist. Die Karte rendert jede Zwischenstufe automatisch neu, wodurch eine flüssige Übergangsbewegung entsteht. Dabei bleibt die Kartenmitte konstant auf die relevante Region ausgerichtet, sodass keine zusätzliche Verschiebung (Pan-Bewegung) erforderlich ist.
Der folgende Codeausschnitt zeigt die technische Umsetzung dieser Zoom-Animation:

\begin{lstlisting}
  [
caption={Zoom-Animation im Einzelspieler-Modus},
label={lst:zoomAnimation}
] 
...
\begin{lstlisting}
  [
caption={Zoom- und Zentrierungslogik im Mehrspieler-Modus},
label={lst:zoomMultiplayer}
] 
function initMap() {
...
Die Darstellung ist dabei rein visuell unterstützend und beeinflusst die eigentliche Spielmechanik nicht. Sie dient ausschließlich der Orientierung innerhalb des gewählten Modus. Da die Polygone direkt auf der Karte gerendert werden, passen sie sich automatisch an Zoomstufe und Kartenausschnitt an. Dadurch bleiben die Grenzen sowohl in der Gesamtansicht als auch in detaillierten Zoomstufen konsistent und klar erkennbar.
Im folgenden Codeabschnitt wird die technische Umsetzung dieser Begrenzungslinien dargestellt:
\begin{lstlisting}
  [
caption={Darstellung von roten Begrenzungslinien der Bundesländer},
label={lst:bundeslandBorders}
] 
/**
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{content/img/4_Technische_Umsetzung/Rote_Begreunzungslinie.jpeg}
    \caption{Begrenzungslinien für Nieserösterreich und Wien}
    \label{fig:beispielabbildung}
\end{figure}
...
\begin{lstlisting}
  [
caption={Beispielhafte Koordinatenliste für die Umrandung von Niederösterreich},
label={lst:niederoesterreich}
] 
...
\mbox{}\\
Der Kompass in der Spielansicht ist ein wichtiges UI-Feature, weil er dem Spieler jederzeit Orientierung gibt und die Street-View-Perspektive lesbarer macht. Da sich die Kamera in Google Street View frei drehen lässt, liefert der Kompass eine direkte Rückmeldung über die Blickrichtung (Heading) und verhindert, dass sich Nutzer beim Erkunden oder beim Platzieren eines Tipps verlaufen. Zusätzlich erhöht das Feature die Immersion, weil es die reale Blickrichtung aus Street View in ein klares, spieltypisches \gls{overlay} übersetzt, ein kleines Detail, das sich für den Nutzer aber als hilfreich darstellt und das Gameplay spürbar verbessert.

Technisch wird der Kompass als fest positioniertes \gls{overlay} über der Street-View-Ansicht dargestellt. Er ist unabhängig vom eigentlichen Karten- bzw. Street-View-Element im Layout verankert und bleibt somit stets sichtbar, auch wenn sich die Perspektive dynamisch ändert. Die Rotation des Kompasszeigers erfolgt synchron zur aktuellen Blickrichtung der Street-View-Kamera, indem der jeweilige Heading-Wert ausgelesen und in eine entsprechende \gls{css}-Transformation übersetzt wird. Dadurch entsteht eine direkte visuelle Kopplung zwischen Kamerabewegung und UI-Element.
...
\begin{lstlisting}
  [
caption={Kompass-Overlay in der Spielansicht},
label={lst:compassOverlay}
] 
/**
 * CompassOverlay
...
const styles = StyleSheet.create({
  // Positionierung als Overlay ( oben links)
  compassOverlay: {
   ...
  },
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{content/img/4_Technische_Umsetzung/Kompass.jpeg}
    \caption{Kompass in der Spieleansicht}
    \label{fig:beispielabbildung}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{content/img/4_Technische_Umsetzung/Supabase_Auth.png}
    \caption{Benutzerverwaltung und Authentifizierungsübersicht in Supabase}
    \label{fig:supabase-auth}
\end{figure}

Abbildung \ref{fig:supabase-auth} zeigt die Authentifizierungsoberfläche von Supabase für einen registrierten Benutzer. 
Dargestellt sind zentrale Metadaten wie die eindeutige Benutzer-ID, Zeitstempel für Registrierung, Bestätigung und letzte Anmeldung sowie der verwendete Authentifizierungsprovider. 
Zusätzlich bietet die Oberfläche administrative Funktionen wie Passwort-Zurücksetzung oder Magic-Link-Anmeldung.

Diese Ansicht verdeutlicht, wie Supabase die Benutzerverwaltung zentral kapselt und sicherheitsrelevante Prozesse serverseitig organisiert. 
Die Anwendung greift ausschließlich über autorisierte Sessions auf diese Daten zu, wodurch eine konsistente und sichere Authentifizierung gewährleistet wird.
...
Durch dieses \gls{publish-subscribe} wird eine latenzarme und skalierbare Synchronisation zwischen mehreren Benutzern ermöglicht. 
...
Ein typisches Beispiel für die Struktur einer über einen Channel übertragenen Nachricht ist in Listing~\ref{lst:realtime-json} dargestellt. 
Dieses JSON zeigt vereinfacht, wie Benutzerdaten oder Statusänderungen zwischen Client und Backend ausgetauscht werden.

\begin{lstlisting}[caption={Beispielhafte Echtzeit-Nachricht über einen Supabase Channel}, label={lst:realtime-json}]
{
  "eventType": "UPDATE",
  "table": "users",
  "new": {
    "id": "dadeb4e7-8143-4103-a114-efc5b2f7baa9",
    "username": "Kliv",
    "email": "l.pichler@htlkrems.at",
    "status": "online",
    "updated_at": "2026-01-26T08:50:08Z"
  }
}
\end{lstlisting}

Das Listing verdeutlicht, dass neben dem Ereignistyp auch die betroffenen Datensätze übertragen werden. 
Die Anwendung kann diese Informationen unmittelbar verarbeiten und die Benutzeroberfläche entsprechend aktualisieren.

\begin{lstlisting}[
caption={Realtime-Subscription zur Synchronisation von Freundschaftsanfragen},
label={lst:friend-realtime}
]
...
\begin{lstlisting}[
caption={Polygonmodell der österreichischen Staatsgrenze zur Positionsgenerierung},
label={lst:austria-polygon}
]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{content/img/4_Technische_Umsetzung/Lobby_Einladung.jpeg}
    \caption{Darstellung einer Live-Einladung in der Benutzeroberfläche}
    \label{fig:live-einladung}
\end{figure}

Abbildung \ref{fig:live-einladung} zeigt die visuelle Darstellung einer eingehenden 
Live-Einladung für den eingeloggten Benutzer. Sobald eine neue Einladung erstellt wird, 
...
\begin{lstlisting}[
caption={Realtime-Subscription zur Verarbeitung eingehender Lobby-Einladungen},
label={lst:lobby-invite-subscription}
]
useEffect(() => {
  // Vorbedingung:
...
\begin{lstlisting}[
caption={Distanzberechnung mittels Haversine-Formel},
label={lst:haversine}
]
