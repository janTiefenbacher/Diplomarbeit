...
Das Mockup dient damit nicht nur als visuelle Vorschau, sondern als konkrete Umsetzung der zuvor definierten UI/UX- und Designprinzipien und bildet die Grundlage für die technische Realisierung der Anwendung.
...
Darüber hinaus müssen alle Marker gleichzeitig auf derselben Karteninstanz gerendert werden. Die Karte dient somit als gemeinsame visuelle Referenzfläche, auf der sämtliche Schätzungen parallel angezeigt werden. Dabei ist darauf zu achten, dass sich Marker bei nah beieinanderliegenden Positionen nicht vollständig überdecken. Durch eine gezielte Steuerung der Rendering-Reihenfolge (z. B. über zIndex) kann sichergestellt werden, dass relevante Marker, etwa der eigene Tipp, stets sichtbar bleiben.

Im Gegensatz zum Einzelspieler-Modus, bei dem ein Spieler ausschließlich seinen eigenen Pin setzt und dessen Position am Ende einsehen kann, erfordert der Mehrspieler-Modus die gleichzeitige Darstellung mehrerer Pins. Diese Funktionalität ist notwendig, um den Spielern die Vermutungen ihrer Mitspieler sichtbar zu machen und einen direkten Vergleich mit der eigenen Schätzung zu ermöglichen.
...
Eine zentrale Voraussetzung für die Anzeige mehrerer Pins ist der nachfolgend dargestellte Ablauf:\begin{enumerate}
