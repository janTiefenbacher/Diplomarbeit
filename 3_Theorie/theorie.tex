...
\subsubsection{Skalierung: vertikal und horizontal}

Skalierung beschreibt die Fähigkeit eines Systems, steigende Last zu bewältigen. Dabei wird zwischen vertikaler und horizontaler Skalierung unterschieden. Bei der vertikalen Skalierung (Scaling Up) wird die Leistungsfähigkeit einer einzelnen Maschine erhöht, beispielsweise durch mehr CPU, Arbeitsspeicher oder schnelleren Speicher. Dieser Ansatz ist oft einfacher umzusetzen, stößt jedoch an technische und wirtschaftliche Grenzen, da einzelne Systeme nur begrenzt ausgebaut werden können \cite{akamai_scaling}.

Horizontale Skalierung (Scaling Out) erweitert ein System durch zusätzliche Maschinen oder Knoten. Die Last wird dabei auf mehrere Instanzen verteilt, wodurch eine flexible Erweiterung möglich ist. Dieses Modell ist besonders für verteilte Systeme und Cloud-Umgebungen relevant, erfordert jedoch meist zusätzliche Mechanismen wie Load Balancer und geeignete Architekturprinzipien \cite{akamai_scaling}.
